...
from app.services.app_service import run_prediction
...
async def scan_url_endpoint(
    ...
    if model_load_error or not all([model, tokenizer, device]):
        raise HTTPException(
            ...
            detail=f"ML model is not ready. Reason: {model_load_error or 'Unknown'}"
        )

    url_to_scan = str(request_data.url)
    
    # Pass user_id to the service function if you want to log scans per user
    prediction_result = run_prediction(url_to_scan, model, tokenizer, device)
